...
{
  const buttonElement = document.querySelector('.Follow-item');
  if(buttonElement.innerHTML === 'Follow')
  {
...
function convertToPDF() {
  ...
  if (fileInput.files.length > 0) {
      ...
      simulateConversion(fileName)
          .then(result => {
              resultElement.innerHTML = `Conversion successful! Download your PDF <a href="${result}" download>here</a>.`;
          })
          .catch(error => {
              resultElement.innerHTML = `Error during conversion: ${error}`;
          });
  } else {
      resultElement.innerHTML = 'Please select a Word file (.docx) before converting.';
  }
}
...
function simulateConversion(fileName) {
  ...
  return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
          const pdfUrl = `path/to/converted/${fileName.replace('.docx', '.pdf')}`;
          resolve(pdfUrl);
      }, 2000);
  });
}
